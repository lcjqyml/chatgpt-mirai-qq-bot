...
class Response(BaseModel):
    error_format: str = "出现故障！如果这个问题持续出现，可以尝试以下方法：\n" \
                        "1. 发送“重置会话” 尝试新会话; \n" \
                        "2. 发送 “回滚对话” 来回溯到上一条对话;\n" \
                        "3. 尝试其他接口如：“v1”、“v3”、“poe”等。\n{exc}"
    ...
    error_server_overloaded: str = "抱歉，当前服务器压力有点大，请等几秒再找我吧！"
    """服务器提示 429 错误时的回复 """
    ...
    reset_chat = "会话已重置，当前为聊天模式，无交互2小时后自动重置为问答模式（省钱模式-_-!），也可直接输入“问答模式”手动切换。AI默认设定：{system_prompt}"
    """重置为聊天模式后发送的消息"""
